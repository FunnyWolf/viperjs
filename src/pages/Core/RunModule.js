...
export const getModuleOptions = (postModuleConfigActive) => {
  const options = []
...
export const changePin = loadpath => {
  const pins = getPins()
      ...
      <Col span={8}>
        <Card bordered={false}>
          ...
          >
            ...
            <Radio.Button
              value="AI_Agent">{formatText('app.runmodule.postmodule.moduletype.AI_Agent')}</Radio.Button>
          </Radio.Group>
          <Table
            style={{
  ...
  const columns = [
    {
      ...
      title: formatText('app.runmodule.botmodule.ip_list'), dataIndex: 'ip_list', key: 'ip_list', width: 96, render: (text, record) => {
        return (<Popover
          ...
          content={<List
            style={{
              padding: '0 0 0 0', overflow: 'auto', maxHeight: cssCalc('50vh'), minHeight: cssCalc('50vh'),
            }}
            size="small"
            bordered
