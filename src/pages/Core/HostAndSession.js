...
import { DocIconInDiv, DocIconInDivSessionIO, DocIconLeft, host_type_to_avatar_table, MyIcon, SidTag } from '@/pages/Core/Common';
...
import { VGPTWindowMemo } from '@/pages/Core/GPT'
...
const HostAndSession = props => {
  ...
  const {
    ...
    setLlmModuleOptions: model.setLlmModuleOptions,
  }));
  ...
  const initHeartBeat = () => {
    ...
    ws.current.onmessage = event => {
      ...
      if (module_options_update) {
        setPostModuleOptions(module_options.filter(item => item.BROKER.indexOf('post_') === 0));
        setBotModuleOptions(module_options.filter(item => item.BROKER.indexOf('bot_') === 0));
        setProxyHttpScanModuleOptions(module_options.filter(item => item.BROKER.indexOf('proxy_') === 0));
        setLlmModuleOptions(module_options.filter(item => item.BROKER.indexOf('llm_') === 0));
      }
    };
  };
...
const HostAndSessionCard = () => {
  ...
  return (<Fragment>
    <DocIconLeft url="https://www.yuque.com/vipersec/help/cr9w1rgb3pyvvnt1"/>
    <Flex>
      <Table
        loading={!heatbeatsocketalive}
...
const FloatingButtons = () => {
  ...
  const [show_gpt_window, set_show_gpt_window] = useState(false);
  ...
  return <Fragment>
    {show_gpt_window ? <NewWindow
      height={window.innerHeight / 10 * 8}
      width={window.innerWidth / 10 * 8}
      title={formatText('app.hostandsession.tab.VGPT')}
      onClose={() => set_show_gpt_window(false)}
    >
      <VGPTWindowMemo/>
    </NewWindow> : null}
    ...
    <Flex vertical>
      <LangSwitch/>
      {show_gpt_window ? <Button
        style={{ width: 40 }}
        danger
        onClick={() => set_show_gpt_window(!show_gpt_window)}
        icon={<OpenAIOutlined/>}
      /> : <Button
        style={{ width: 40 }}
        onClick={() => set_show_gpt_window(!show_gpt_window)}
      ...
      {/*  // href={'#/nav'}*/}
      {/*  target={'_blank'}*/}
      {/*  href="https://www.yuque.com/vipersec/help/cr9w1rgb3pyvvnt1"*/}
      {/*/>*/}
    </Flex>
  </Fragment>;
};
    ...
    <Row>
      ...
      <Table
        ...
        loading={createPostModuleActuatorReq.loading || listFileSessionReq.loading || listFileSessionRunReq.loading || updateFileSessionReq.loading ||
          listFileSessionCatReq.loading || createFileSessionReq.loading || destoryFileSessionReq.loading}
        onRow={record => ({
          onDoubleClick: event => {
