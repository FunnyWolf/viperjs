  ...
  const handlerPayloadSpecialOption = () => {
    ...
    if (selectPayload.includes('reverse_http') || selectPayload.includes('reverse_winhttp')) {
      ...
      options.push(<FormNew.Item
        ...
        label={
          <Tooltip
            title="An optional value to use for the Host HTTP header">
      ...
      options.push(<FormNew.Item
        ...
        label={
          <Tooltip
            title="When OverrideRequestHost is set, use this value as the host name for secondary requests">
      ...
      options.push(<FormNew.Item
        ...
        label={
          <Tooltip
            title="When OverrideRequestHost is set, use this value as the port number for secondary requests">
    ...
    if (selectPayload.includes('rc4')) {
      ...
      options.push(<FormNew.Item {...formLayout} label="说明">
      ...
      options.push(
        <Row>
          <Col span={14} offset={6}>
            <Alert message="reverse_http类型payload不免杀,建议使用reverse_https类型" type="warning" showIcon/>
          </Col>
        </Row>);
    }
    ...
    if (options.length === 0) {
      ...
      return <Panel header="高级参数" key="advance">{options}</Panel>;
    }
  };
          ...
          },
          {
            ...
            render: (text, record) => {
              ...
              if (record.ID >= 0) {
                transformAction = <a
                  style={{ color: '#faad14' }}
                  onClick={() => onCreateVirtualHandler(record)}>
                  虚拟化
                </a>;
              } else {
                transformAction = <a
